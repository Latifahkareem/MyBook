...
?>
      ...
      <div class="row">
        ...
        <form action="Books.php" method="post" name="addForm" id="addForm" enctype="multipart/form-data" class="form-horizontal" onSubmit="return validate();">
            ...
            <div>
              <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped">
                  ...
                  <?php 
		  $sql_list = "SELECT *  FROM  books b  inner join courses c on  c.course_id = b.course_id ORDER BY book_id " ;
		  // echo $sql_list ; 
		  ...
		  $i=1 ; 
			while($q_list = mysql_fetch_array($r_list))
			{
		  	...
		  	<tr>
                      ...
                      <td>
                      <a href="?act=del&id=<?=$q_list[book_id]?>"  title="Delete" ><i class="fa fa-trash" aria-hidden="true"></i></a>     
                      <a href="?act=edit&id=<?=$q_list[book_id]?>"  title="Edit" > <i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
...
function validate()
{
	...
	if(!letters.test($('#book_name').val())) {
        alert("Error: Book  Name  must contain only letters (A-Za-z)!");
        $('#book_name').focus();
        return false;
      }
	 
	 var numbers = /^[0-9]+$/;
	 if(!numbers.test($('#ispn').val())) {
        alert("Error: ispn  must contain only numbers (0-9)!");
        $('#ispn').focus();
        return false;
      }
